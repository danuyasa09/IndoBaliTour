...
@section('subtitle', 'Ubah data armada kendaraan')
    ...
    <form action="{{ route('admin.cars.update', $car->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="md:col-span-2">
                ...
                <input type="text" name="title" id="title" value="{{ old('title', $car->title) }}" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-brand-red focus:ring-brand-red" required>
            ...
            <div>
                ...
                <select name="status" id="status" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-brand-red focus:ring-brand-red" required>
                    <option value="Show" {{ old('status', $car->status) == 'Show' ? 'selected' : '' }}>Tampilkan (Show)</option>
                    <option value="Hide" {{ old('status', $car->status) == 'Hide' ? 'selected' : '' }}>Sembunyikan (Hide)</option>
                </select>
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="content" id="content" rows="6" class="summernote w-full rounded-lg border-gray-300 shadow-sm focus:border-brand-red focus:ring-brand-red" required>{{ old('content', $car->content) }}</textarea>
            ...
            <div class="md:col-span-2">
                ...
                @if($car->img)
                    <div class="mb-3">
                        <img src="{{ asset('images/cars/' . $car->img) }}" alt="Foto Kendaraan" class="h-32 rounded-lg object-cover">
                    </div>
                @endif
