...
@section('title', 'Edit Airport Transfer')
@section('subtitle', 'Ubah rute dan harga transfer bandara')
    ...
    <form action="{{ route('admin.airports.update', $airport->id) }}" method="POST">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" name="start" id="start" value="{{ old('start', $airport->start) }}" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-brand-red focus:ring-brand-red" required>
            ...
            <div>
                ...
                <input type="text" name="destination" id="destination" value="{{ old('destination', $airport->destination) }}" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-brand-red focus:ring-brand-red" required>
            ...
            <div>
                ...
                <select name="status" id="status" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-brand-red focus:ring-brand-red" required>
                    <option value="Show" {{ old('status', $airport->status) == 'Show' ? 'selected' : '' }}>Tampilkan (Show)</option>
                    <option value="Hide" {{ old('status', $airport->status) == 'Hide' ? 'selected' : '' }}>Sembunyikan (Hide)</option>
                </select>
