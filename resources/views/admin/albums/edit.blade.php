...
@section('subtitle', 'Ubah data album')
    ...
    <form action="{{ route('admin.albums.update', $album->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="md:col-span-2">
                ...
                <input type="text" name="title" id="title" value="{{ old('title', $album->title) }}" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-brand-red focus:ring-brand-red" required>
            ...
            <div>
                ...
                <input type="date" name="date" id="date" value="{{ old('date', $album->date ? $album->date->format('Y-m-d') : '') }}" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-brand-red focus:ring-brand-red" required>
            ...
            <div>
                ...
                <select name="status" id="status" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-brand-red focus:ring-brand-red" required>
                    <option value="Show" {{ old('status', $album->status) == 'Show' ? 'selected' : '' }}>Tampilkan (Show)</option>
                    <option value="Hide" {{ old('status', $album->status) == 'Hide' ? 'selected' : '' }}>Sembunyikan (Hide)</option>
                </select>
            ...
            <div class="md:col-span-2">
                ...
                @if($album->img)
                    <div class="mb-3">
                        <img src="{{ asset('images/albums/' . $album->img) }}" alt="Cover" class="h-32 rounded-lg object-cover">
                    </div>
                @endif
                ...
                <p class="mt-1 text-xs text-gray-500">Abaikan jika tidak ingin mengubah cover. Format: JPG, PNG, max 2MB.</p>
